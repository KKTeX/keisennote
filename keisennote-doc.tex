...
\begin{itemize}
  ...
  \begin{SourceCode}{TeX}
    \noteLineWidth@kn=\kn@linewidth\relax
    \noteDotsRadius@kn=\kn@radius\relax
    \noteLineDistance@kn=\kn@distance\relax
    \noteTriangle@mag@kn=\kn@triangle\relax
  \end{SourceCode}
...
\begin{itemize}
  ...
  \begin{SourceCode}{TeX}
    ...
    \noteLineWidth@kn=\kn@linewidth\relax  % reassign to internal registers if needed
  \end{SourceCode}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{v1.1.0 (2025/12/30)} --- Changed License.
\end{itemize}
...
\section{Source Code}
  \begin{lstlisting}
    ...
    \newdimen\noteDotsRadius@kn
    \noteDotsRadius@kn=.8truept

    \newdimen\noteLineDistance@kn
    \noteLineDistance@kn=6truemm

    \newdimen\noteTriangle@mag@kn
    \noteTriangle@mag@kn=.5pt
        ...
        \begin{tikzpicture}[xscale=\VDNT@xscaler]
          ...
          \@whiledim\VDNT@currentYPos<\VDNT@notegoal\do{
            ...
            \foreach \k in{0,1,...,\VDNT@dotsNum}{%
              ...
              \fill[#1] (\VDNT@currentXPos,\VDNT@currentYPos) circle [radius=\noteDotsRadius@kn];
            }
            \advance\VDNT@currentYPos\VDNT@Yinterval\relax
          }
          \fill[#1] (\VDNT@Xinterval*\VDNT@dotsNum/2,\VDNT@currentYPos-\VDNT@Yinterval-\noteTriangle@mag@kn*4pt) -- ++(\noteTriangle@mag@kn*3pt,\noteTriangle@mag@kn*4pt) -- ++(-\noteTriangle@mag@kn*6pt,0) -- cycle;
        \end{tikzpicture}%
      }%
      \egroup
      %%座標管理用カウンタのインクリメント
    ...
    \NewDocumentCommand{\note}{ m O{white!70!black} }{\par\bgroup
      ...
      \noindent
        \begin{tikzpicture}[xscale=\VDNT@xscaler]
          \VDNT@currentYPos\z@
          \fill[#2] (\VDNT@Xinterval*\VDNT@dotsNum/2,\VDNT@currentYPos+\VDNT@Yinterval+\noteTriangle@mag@kn*4pt) -- ++(\noteTriangle@mag@kn*3pt,-\noteTriangle@mag@kn*4pt) -- ++(-\noteTriangle@mag@kn*6pt,0) -- cycle; %上の三角形
          \foreach \i in{1,2,...,#1}{ 
            \VDNT@currentXPos\z@
            ...
            \foreach \k in{0,1,...,\VDNT@dotsNum}{
              \VDNT@currentXPos=\dimexpr\VDNT@Xinterval*\k\relax
              \fill[#2] (\VDNT@currentXPos,\VDNT@currentYPos) circle [radius=\noteDotsRadius@kn];
            }
          }
          \fill[#2] (\VDNT@Xinterval*\VDNT@dotsNum/2,\VDNT@currentYPos-\noteTriangle@mag@kn*4pt) -- ++(\noteTriangle@mag@kn*3pt,\noteTriangle@mag@kn*4pt) -- ++(-\noteTriangle@mag@kn*6pt,0) -- cycle; %下の三角形
        \end{tikzpicture}%
      \egroup
      \par
    }
